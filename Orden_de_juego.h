...
int orden1(int ronda,string NombreP1, string NombreP2,int Trufas_Totales_de_Jugador[]){
...
int orden2(int ronda,string NombreP1, string NombreP2,int Trufas_Totales_de_Jugador[]){
    if(ronda%2==0||ronda==0){
        rlutil::locate(10,1);
        cout<<" TURNO DEL JUGADOR "<<NombreP2<<": "<<endl;
         rlutil::locate(40,1);
...
bool Quien_emppieza(string NombreP1,string NombreP2,int dados[], int cantidadDeDados,int valorDeDadoJugador1[],int valorDeDadoJugador2[]){
   ...
   system("pause");
    ...
    cout << "Valores de los dados del Jugador " << NombreP1 << endl <<": "<<endl;
    for (int i = 0; i < cantidadDeDados; ++i) {
...
// Mostrar resultados de los dados del jugador 2
    cout << "Valores de los dados del Jugador "<< NombreP2 << endl <<": "<<endl;
    for (int i = 0; i < cantidadDeDados; ++i) {
...
// Compara los valores de los dados de cada jugador
    if ((valorDeDadoJugador1[0] > valorDeDadoJugador1[1] && valorDeDadoJugador1[0] > valorDeDadoJugador2[0] && valorDeDadoJugador1[0] > valorDeDadoJugador2[1]) ||
        ...
        cout << "Comienza el cerdito " << NombreP2 << "!" << endl;
        return false;}
        else { cout << "¡Hay un empate en el puntaje y el dado mas alto! ¡Vuelvan a tirar!" << endl;}
    }
    }
